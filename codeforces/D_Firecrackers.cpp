...
int check(vi &limit, int num, int a, int b, vi &ind)
{
    int timeRemaining = limit[num - 1] - (num-ind[num-1]-1);
    debug3(num, limit[num-1], timeRemaining);
    b = max(0, b - num);
    if (timeRemaining < 0)
    {
        a = max(0, a + timeRemaining);
        debug2(a,b);
        if (a >= b)
            return 0;
        return 1;
    }
    b = max(0, b - timeRemaining);
    a = max(0, a - timeRemaining);
    debug2(a,b);
    if (a >= b)
        return 0;
...
void solve()
{
    ...
    ind[0] = 0;
    for (int i = 1; i < m; i++) {
        if(arr[i]+i > arr[i-1]) ind[i] = i;
        else ind[i] = ind[i-1];
        arr[i] = max(i + arr[i], arr[i - 1]);
    }
    int low = 0, high = m, ans = 0;
    while (low <= high)
    {
        int mid = (low + high) / 2;
        if (check(arr, mid, a, b, ind))
            ans = mid, low = mid + 1;
        else
            high = mid - 1;
    }
