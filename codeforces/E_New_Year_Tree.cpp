...
public:
    ...
    void forward(int start, int end, int index) {
        if(lazyTree[index]!=0) {
            ...
            if(start!=end) {
                ...
                lazyTree[index + 2 * ((start + end) / 2 - start + 1)] = lazyTree[index];
            }
            lazyTree[index] = 0;
        }
    }
    ...
    T lazyQuery(T l, T r)
    {
        ll orvalue = lazyQuery(0, 0, this->n - 1, l, r);
        debug(orvalue);
        return __builtin_popcountll(orvalue);
    }
...
void solve()
{
    ...
    while(m--) {
        ...
        if(sign == 1) {
            ll index, val;
            cin>>index>>val;
            segtree.update(tin[index], tout[index], val);
        } else {
            ll index;
            cin>>index;
            cout<<segtree.lazyQuery(tin[index], tout[index])<<endl;
        }
    }
}
