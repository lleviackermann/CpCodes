...
void solve()
{
    ...
    vl power(ma, 1);
    ll mod2 = 1e9+9;
    for(int i = 1; i < ma; i++) power[i] = power[i-1] * p % mod, power2[i] = power2[i-1] * p2 % mod2;
    auto calcHash = [&](map<ll, ll> &cnt) -> pl
    {
        ll temp = 0;
        ll t2 = 0;
        for (auto [fir, sec] : cnt)
        {
            temp = temp + (fir * sec % mod) * power[fir] % mod;
            temp = temp % mod;
            t2 = t2 + (fir * sec % mod2) * power2[fir] % mod2;
            t2 = t2 % mod;
        }
        return {temp, t2};
    };
    ...
    for(auto num : arr) {
        ...
        if(x > 1) cnt[x]++;
        cnt[x] = cnt[x] % k;
        pl curr = calcHash(cnt);
        print(cnt);
        // debug(curr);
        for(auto& [fir, sec] : cnt) sec = (k - sec) % k;
        pl tocal = calcHash(cnt);
        ans += hash[tocal];
        print(cnt);
        hash[curr]++;
    }
