...
vl factorial(nmax), inv_fact(nmax);
...
void precalc()
{
    ...
    inv_fact[nmax - 1] = binpow(factorial[nmax - 1], mod - 2, mod);
    for (ll i = nmax - 2; i >= 0; i--)
...
ll nCr(ll n, ll r)
{
    ...
    if (r > n || r < 0)
        return 0;
    ll ans = factorial[n] * inv_fact[r] % mod;
...
void solve()
{
    ...
    for (ll i = 2; i <= n; i++)
    {
        ...
        {
            if(i - k > k - 1) continue;
            ll temp = nCr(i - 1, i - k) * nCr(n - i, 2 * k - i - 1) % mod;
            temp = temp * i % mod;
            // debug3(i, k, temp);
            ans = (ans + temp) % mod;
        }
