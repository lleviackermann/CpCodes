...
int binfunc(vpi &messages, int k, int l)
{
    if(k == 1) {
        for(auto i : messages) {
            if(i.ff <= l) return 1;
    ...
    int n = messages.size();
    for(int i = 0; i <= n-k; i++) {
        int low = messages[i].ss;
        int high;
        priority_queue<ll> store;
        ll sum = messages[i].ff;
        for(int j = i+1; j < n; j++) {
            while(store.size() > k-2) {
                sum -= store.top();
                store.pop();
            }
            high = messages[j].ss;
            sum += messages[j].ff;
            if(sum + high - low <= l && store.size() == k-2) return 1;
            store.push(messages[j].ff);
        }
    }
    return 0;
}
...
void solve()
{
    ...
    sort(all(messages), [&](pi fir, pi sec)
         {
        if(fir.ss == sec.ss) return fir.ff < sec.ff;
        return fir.ss < sec.ss; });
    // for (auto i : messages)
    ...
    {
        int mid = (l + r) / 2;
        int check = binfunc(messages, mid, left);
        if (check)
            ans = mid, l = mid + 1;
        else
            r = mid - 1;
