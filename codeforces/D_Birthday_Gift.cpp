...
void solve()
{
    ...
    for (int i = 29; i >= 0; i--)
    {
        ...
        if (i == 29)
        {
            ...
            if(setbit && !flag) store.pb(temp.size());
            if (setbit) {
                for(int j = 0; j < n; j++) temp.insert(j);
            }
            end = temp;
            print(end);
            continue;
        ...
        for (auto num : arr)
        {
            int bit = (num >> i) & 1;
            flag = flag ^ bit;
            auto it = end.lower_bound(counter);
            if (it == end.end())
            {
                debug(bit);
                cout << "-1\n";
    ...
    print(store);
    int ma = -1;
    if(store.size()) ma = *max_element(all(store));
    ma = (ma == 0) ? -1 : ma;
    int finans = max(ma, (int)end.size());
    if(finans == 0) finans = -1;
    cout << finans << endl;
}
