...
void solve()
{
    ...
    for(int i = 0; i < n; i++) {
        string temp = "";
        for(int j = i; j < n; j++) temp += s[j], subs.pb({temp, i+1});
    }
    sort(all(subs));
    // for(auto [sub, ind]: subs) cout<<sub<<" "<<ind<<endl;
    vl dp(n+1, 0);
    SegmentTree<ll> segtree(dp);
    ll ans = 0;
    int count = 0;
    for(int i = 0; i < subs.size(); i++) {
        if(i && subs[i].ff != subs[i-1].ff) {
            while(count < i) {
                auto [sub, ind] = subs[count];
                int bac = ind + sub.size() - 1;
                segtree.update(bac, dp[bac]);
                count++;
            }
        }
        auto [sub, ind] = subs[i];
        ll bac = ind + sub.size() - 1;
        ll pre = segtree.query(0, ind-1);
        dp[bac] = max(dp[bac], pre + value[sub.size()]);
        ans = max(ans, dp[bac]);
    }
    cout<<ans<<endl;
}
