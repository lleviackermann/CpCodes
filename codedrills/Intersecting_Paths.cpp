...
public:
    ...
    void update(T index, T target, T value, T start, T end)
    {
        ...
        if (target <= mid)
        {
            update(index + 1, target, value, start, mid);
        }
        else
        {
            update(index + 2 * (mid - start + 1), target, value, mid + 1, end);
        }
        tree[index] = tree[index + 1] + tree[index + 2 * (mid - start + 1)];
    }
...
void solve()
{
    ...
    preprocess(n);
    vi dp(timer + 2, 0);
    SegmentTree<int> lca_count(dp);
    SegmentTree<int> path_count(dp);
    ll ans = 0;
    auto get_intersection = [&](int u, int v, int lc) {
        ll ret = lca_count.query(0, tin[u]);
        ret += lca_count.query(0, tin[v]);
        ret -= 2ll * lca_count.query(0, tin[lc]);
        ret += lcacount[lc] - 1;
        ret += path_count.query(tin[lc], tout[lc]);
        return ret;
    };

    auto path_add_remove = [&](int u, int v, int lc, int count) {
        lca_count.update(tin[lc], count);
        lca_count.update(tout[lc], -1 * count);
        path_count.update(tin[u], count);
        path_count.update(tin[v], count);
        path_count.update(tin[lc], -2 * count);
    };
    ...
    for(int i = 0; i < q; i++) {
        ...
        int lc = lca(l, r);
        if(store.count({l,r})) {
            ans -= get_intersection(l, r, lc);
            path_add_remove(l, r, lc, -1);
            lcacount[lc]--;
            store.erase({l,r});
        } else {
            lcacount[lc]++;
