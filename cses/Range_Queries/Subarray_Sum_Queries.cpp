...
struct item {
    ll ans, prefix, suffix, total;
    item() {}

    item(ll n) {
        ans = max(0ll, n), total = n, prefix = n, suffix = n;
    }

    static item merge(item &first, item &second) {
        item temp;
        temp.ans = max({first.ans, second.ans, first.total + max(second.total, second.prefix), 
                        first.suffix + max(second.total, second.prefix)});
        temp.prefix = max({first.prefix, first.total + max(second.total, second.prefix)});
        temp.suffix = max({second.suffix, max(first.total, first.suffix) + second.total});
...
public:
    ...
    V query(T index, T start, T end, T l, T r)
    {
        if (start > r || end < l)
        {
            return V(0);
        ...
        // debug2(start, end);
        T mid = (start + end) / 2;
        V first = query(2 * index + 1, start, mid, l, r);
        V second = query(2 * index + 2, mid + 1, end, l, r);
        debug2(start, end);
        debug4(first.ans, first.total, second.ans, second.total);
        V temp = V::merge(first, second);
        debug2(temp.ans, temp.total);
