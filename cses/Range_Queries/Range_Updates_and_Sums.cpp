...
public:
    ...
    {
        if (start != end)
            forward(index, start, end);
        else
        {
            if (lazyTree[index] < 0)
                tree[index] = -1 * lazyTree[index];
            else
    ...
    void update(T index, T l, T r, T value, T start, T end, int flag)
    {
        if (start != end)
            forward(index, start, end);
        else
        ...
        if (start >= l && end <= r)
        {
            setinc(index, value, flag);
            forward(index, start, end);
            return;
        }

        T mid = (start + end) / 2;
        update(2 * index + 1, l, r, value, start, mid, flag);
        update(2 * index + 2, l, r, value, mid + 1, end, flag);
        tree[index] = tree[2 * index + 1] + tree[2 * index + 2];
    }

    void forward(T ind, T st, T en)
    {
        if (lazyTree[ind] < 0)
        {
            tree[ind] = (en - st + 1) * -1 * lazyTree[ind];
            lazyTree[ind] = -1 * lazyTree[ind];
            setinc(2 * ind + 1, lazyTree[ind], 1);
            setinc(2 * ind + 2, lazyTree[ind], 1);
            lazyTree[ind] = 0;
        }
        else if (lazyTree[ind] > 0)
        {
            tree[ind] = tree[ind] + (en - st + 1) * lazyTree[ind];
            setinc(2 * ind + 1, lazyTree[ind], 0);
            setinc(2 * ind + 2, lazyTree[ind], 0);
            lazyTree[ind] = 0;
        }
    }

    void setinc(T ind, T val, T flag)
    {
        if (flag)
        {
            lazyTree[ind] = -1 * val;
        ...
        {
            if (lazyTree[ind] < 0)
                lazyTree[ind] = -1 * (-1 * lazyTree[ind] + val);
            else
                lazyTree[ind] += val;
        }
    }
    ...
    void update(T l, T r, T value, T flag)
    {
        update(0, --l, --r, value, 0, this->n - 1, flag);
    }
};
...
void solve()
{
    ...
    while (q--)
    {
        ...
        else
        {
            ll val;
            cin >> val;
            segtree.update(l, r, val, fir == 2 ? 1 : 0);
        }
    }
}
