...
void solve()
{
    ...
    SegmentTree<ll> segtree(sum);
    SegmentTree<ll> another(count);
    for(ll i = 0; i < n; i++) {
        oset.insert({arr[i], i});
        if(i >= k) {
            segtree.update(ma[arr[i]]+1, arr[i]);
            another.update(ma[arr[i]]+1, 1);
            oset.erase({arr[i-k], i-k});
            segtree.update(ma[arr[i-k]]+1, -1 * (arr[i-k]));
            another.update(ma[arr[i-k]]+1, -1);
        }
        if(i >= k-1) {
            ll median = (*(oset.find_by_order((k-1)/2))).first;
            ll te = ma[median];
            ll left = another.query(1, te) * median - segtree.query(1, te);
            ll right = segtree.query(te+2, 2e5+1) - median * another.query(te+2, 2e5+1);
            cout<<left+right<<" ";
